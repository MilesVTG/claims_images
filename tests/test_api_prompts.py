...
import pytest
from tests.conftest import seed_test_prompt
...
class TestListPrompts:
    ...
    def test_list_prompts_filter_category(self, test_client, db_session):
        ...
        resp = test_client.get("/api/prompts?category=analysis")
        assert resp.status_code == 200
        data = resp.json()
        for p in data:
    ...
    def test_list_prompts_include_inactive(self, test_client, db_session):
        ...
        resp_active = test_client.get("/api/prompts?active_only=true")
        resp_all = test_client.get("/api/prompts?active_only=false")
...
class TestGetPrompt:
    ...
    def test_get_prompt_by_slug(self, test_client, db_session):
        ...
        resp = test_client.get("/api/prompts/get_me")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_get_prompt_not_found(self, test_client):
        resp = test_client.get("/api/prompts/nonexistent_slug")
...
class TestCreatePrompt:
    ...
    def test_create_prompt_missing_fields(self, test_client):
        resp = test_client.post("/api/prompts", json={"slug": "only_slug"})
...
class TestUpdatePrompt:
    ...
    def test_update_prompt_content(self, test_client, db_session):
        ...
        get_resp = test_client.get("/api/prompts/update_me")
    ...
    def test_update_prompt_creates_history(self, test_client, db_session):
        ...
        test_client.patch("/api/prompts/hist_prompt", json={"content": "Version 2"})

        resp = test_client.get("/api/prompts/hist_prompt")
        data = resp.json()
        assert data["version"] == 2
    ...
    def test_update_prompt_not_found(self, test_client):
        resp = test_client.patch("/api/prompts/nonexistent", json={"content": "New"})
        assert resp.status_code == 404

    def test_update_prompt_deactivate(self, test_client, db_session):
        seed_test_prompt(db_session, slug="deactivate_me")

        resp = test_client.patch("/api/prompts/deactivate_me", json={"is_active": False})
        assert resp.status_code == 200
